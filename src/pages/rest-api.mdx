...
[`kryptic ci export`](/cli#ci-export), which performs the decryption
chain for you with [Kryptic.Encryption.Go](https://github.com/dev-kryptic/Kryptic.Encryption.Go).
...
[Envelopes](/geeks/envelopes). `PUT` cannot write a Reference or Org
...
<DocsTable
  ...
  rows={[
    ...
    [
      '403',
      "The machine identity's scopes exclude this project, or it has no org-key grant yet",
    ],
    ['404', 'Unknown project public id, environment slug, or secret key'],
    [
      '400',
      'Invalid secret key (must look like an environment variable name) or invalid envelope',
    ],
  ]}
/>
