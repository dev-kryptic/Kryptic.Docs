...
Source: [github.com/dev-kryptic/k8s-operator](https://github.com/dev-kryptic/k8s-operator)
(Apache 2.0). The operator decrypts **inside the cluster** with
...
Create a [machine identity](/machine-identities) in the dashboard, grant it the
organization key under **Settings → Encryption**, then store its credentials in
the namespace where your `KrypticSecret`s will live:
