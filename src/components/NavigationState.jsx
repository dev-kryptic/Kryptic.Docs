...
export function NavigationStateProvider({ children, index }) {
  ...
  const setActiveHighlight = async (groupTitle = undefined) => {
    ...
    const links = []
    document
      .querySelectorAll(`.outer-wrapper-${index} [data-nav-link]`)
      .forEach((link) => {
        links.push(link.getAttribute('data-nav-link'))
      })
    links.shift()

    const activeGroupTitle = groupTitle
      ? groupTitle
      : document
          .querySelector(`.outer-wrapper-${index} [data-nav-active="1"]`)
  ...
  return (
    <NavigationStateContext.Provider value={[activeIndex, setActiveHighlight]}>
      <div className={`outer-wrapper-${index}`}>{children}</div>
